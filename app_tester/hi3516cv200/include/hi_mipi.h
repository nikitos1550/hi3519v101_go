...
typedef enum
{
    ...
    MIPI_HEADER_ERR    = 0x1 << 28,        /* Header has two error at least ,and ECC error correction is invalid */
}MIPI_INT_ERR;
...
typedef enum
{
    ...
    MIPI_VSYNC_ERR   = 0x1 << 21,           /* LVDS mode,Link 0 each lane frame sync signal vsync valid out of sync interrupt status */
}LINK_INT_STAT;
...
typedef enum
{
    ...
    LINK_SYNC_ERR       = 0x3F0000,  /* The line/frame end signal of the link can not detect the interrupt status normally. If there is any interruption, the link will be restarted */
}LVDS_SYNC_INTR_ERR;
