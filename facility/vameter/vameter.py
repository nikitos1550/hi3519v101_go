...
def getDxT(): #TODO use it, now k are hardcoded
    cmd = [ 0x0C,
            0x03,
            0x00, 0x23,
            0x00, 0x01]
    print("query", addcrc(cmd))
    vameter.write(addcrc(cmd))
    answer = vameter.readline()
    answer = list(answer)
    print("answer", answer)
    answer_test = answer[:-2]
    print("expected", addcrc(answer_test))
...
def getV():
    ...
    answer = list(answer)
    #print("answer", answer)
    answer_test = addcrc(answer[:-2])
    #print("expected", answer_test)
    if answer == answer_test:
        v = (answer[3]<<8)+answer[4]
        v = (v/10000)*100
        #print("V=",v)
    else:
        v = -1
...
vameter = serial.Serial(PORT, BAUDRATE, timeout = TIMEOUT)
