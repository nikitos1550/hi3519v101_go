...
VI_DEV_ATTR_S DEV_ATTR_MIPI_BASE_IMX323 = //WORKS ON JVT (N.4?)
{
    // interface mode 
...
int mpp3_vi_init(unsigned int *error_code) {
    *error_code = 0;
    ...
    memset(&stViDevAttr,0,sizeof(stViDevAttr));
            memcpy(&stViDevAttr, &DEV_ATTR_MIPI_BASE_IMX323, sizeof(stViDevAttr)); //ok on jvt, bad on xm
	    //memcpy(&stViDevAttr, &DEV_ATTR_MIPI_BASE, sizeof(stViDevAttr));//DEV_ATTR_MIPI_BASE
