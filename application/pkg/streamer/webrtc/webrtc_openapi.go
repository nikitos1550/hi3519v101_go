...
func init() {
    ...
    openapi.AddApiRoute("connectSessionWebrt",   "/webrtc/connectSession",   "POST",      connectSessionWebrtc)
    openapi.AddApiRoute("disconnectWebrt",   "/webrtc/disconnect",   "GET",      disconnectWebrtc)
}
...
func connectSessionWebrtc(w http.ResponseWriter, r *http.Request) {
	body, err := ioutil.ReadAll(r.Body)
    if err != nil {
		openapi.ResponseErrorWithDetails(w, http.StatusBadRequest, responseRecord{Message: "Failed to read request body"})
		return
